...
#%%old script whith emstool
#compute linear reg on each point
#reg_all_point = esmtools.stats.linear_slope(ds_var, dim = 'time')    
#slope = np.repeat(reg_all_point.sel(parameter = 'slope').values[:,:,np.newaxis],s, axis = 2)
#intercept = np.repeat(reg_all_point.sel(parameter = 'intercept').values[:,:,np.newaxis],s, axis = 2)
